	...
	execute: async (interaction) => {
		...
		if (!roll) {
			return interaction.editReply(
				`<:worng:946623680291696660> | There is no dj role setup in your guild. Try doing \`\`\`/dj_role\`\`\``
			);
		}
		const queue = player.getQueue(interaction.guild.id);
		if (!interaction.member.roles.cache.has(roll)) {
			return interaction.editReply("<:worng:946623680291696660> | You don't have the DJ role!");
		}
