	...
	async execute(interaction) {
		...
		if (!interaction.member.voice.channelId)
			return await interaction.editReply({
				content: "<:worng:946623680291696660> | You need to be in a voice channel!",
				ephemeral: true,
			});
		if (
			...
			return await interaction.editReply({
				content:
					"<:worng:946623680291696660> | You need to be in the same voice channel as me to do that",
				ephemeral: true,
			});
		...
		try {
			...
			queue.destroy();
			return await interaction.editReply({
				content: "<:worng:946623680291696660> | Could not join your voice channel!",
				ephemeral: true,
			});
		}
		...
		if (!track || !track.tracks.length)
			return await interaction.editReply({
				content: `<:worng:946623680291696660> | No Video/Song/Playlist was found when searching for : ${track}`,
				ephemeral: true,
			});
		...
		const playEmbed = new MessageEmbed()
			.setColor(`RANDOM`)
			.setTitle(
				`<a:musicop:946630122881028208> | New ${track.playlist ? "playlist" : "song"} Added to queue`
			);
		if (!track.playlist) {
